...
- **富文本：** Tiptap Vue 3，自定义 `mediaMention` 原子节点和 `@` Suggestion 菜单。
- **状态：** Vue composables；MVP 不引入 Pinia。
- **服务端：** Fastify，负责本地素材、Dry-run、方舟代理和任务轮询。
- **验证：** Vitest、Vue Test Utils、Fastify inject 测试、浏览器人工/自动交互验证。
- **代码组织：** 单仓库，前端与服务端共享 TypeScript 类型和序列化器。
...
- `POST /api/media`：校验并保存本地参考文件。
- `DELETE /api/media/:id`：删除本地文件和记录。
- `POST /api/generations/dry-run`：验证并返回序列化结果；永不访问方舟。
- `POST /api/generations`：二次确认后创建真实任务。
- `GET /api/generations/:id`：查询并规范化任务状态。
- `DELETE /api/generations/:id`：在方舟 API 支持时取消/删除任务。
...
每个目录只承担一个职责。序列化器与共享类型不依赖 Vue 或 Fastify，以便独立测试。

## 11. 完成定义

- `npm install` 后可通过一个开发命令启动前端和服务端。
